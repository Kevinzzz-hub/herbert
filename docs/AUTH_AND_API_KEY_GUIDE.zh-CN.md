...
如果 Herbert 面向所有人开放，而所有 AI 请求都使用开发者自己的 DeepSeek API Key，用户越多，开发者承担的模型费用越高，也很容易被恶意请求耗尽额度。
...
浏览器
  ├── 保存 Supabase 登录会话
  ├── 输入 DeepSeek API Key（只在连接或替换时）
  └── 以后只看到 DeepSeek ••••1234
          │
          ▼ HTTPS
Herbert 服务器
  ├── 验证 Supabase 用户令牌
  ├── 调用 DeepSeek /models 验证 Key
  └── 使用服务器权限访问 Vault
          │
          ▼
Supabase Vault
  └── 加密保存每个用户的 DeepSeek API Key
```

完整密钥不会写入 React state 之外的浏览器存储，也不会在保存后由 Herbert 接口返回。AI 请求发生时，服务器按当前用户 ID 从 Vault 临时取出密钥，在内存中调用 DeepSeek。
...
5. Herbert 用该密钥调用 DeepSeek。
6. DeepSeek 结果返回用户；完整密钥不会出现在响应中。
